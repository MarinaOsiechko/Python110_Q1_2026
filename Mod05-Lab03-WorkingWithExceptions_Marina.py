# ------------------------------------------------------------------------------------------ #
# Title: Mod05-Lab03  Working with exception handling
# Desc: Demonstrates how to use exception handling in your code
# Change Log: (Who, When, What)
#   RRoot,1/1/2030,Created Script Template
...
file = None  # Not using type hint helps PyCharm, so we won't use it going forward,m
csv_data: str = ''

import json
"""students = []   # this creates json file when it doesn't exist
file = open("MyLabData.json", "w")
json.dump(students, file)
file.close()"""

try:
    #Open JSON file, dump data into students table and closes (load converts JSON into Python)
    file = open("MyLabData.json", "r")
...
"""# Define your data, this was used to add names to the dictionary
student_row1: dict[str, float] = {"FirstName": "Bob", "LastName": "Smith", "GPA": 3.5}
student_row2: dict[str, float] = {"FirstName": "Sue", "LastName": "Jones", "GPA": 4.0}
students: list[dict[str, float]] = [student_row1, student_row2]
...
while True:
    ...
    if menu_choice == "1":
        ...
        try:
            ...
            try:   # using a nested try block to capture when an input can not be changed to a float
        ...
        #  Save the data from the dictionary to the file
